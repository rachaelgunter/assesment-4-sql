...
SELECT cupcakes.name AS name, SUM(orders.num_cupcakes) AS sum FROM cupcakes LEFT JOIN ORDERS  ON cupcakes.id = orders.cupcake_id  GROUP BY cupcakes.name ORDER BY name;
...
SELECT customers.email FROM orders LEFT JOIN customers ON customers.id = orders.customer_id GROUP BY customers.email HAVING SUM(num_cupcakes) > 100;
...
SELECT DISTINCT fname, lname, email FROM customers JOIN orders ON customers.id = orders.customer_id JOIN cupcakes ON orders.cupcake_id = cupcakes.id WHERE cupcakes.name = 'funfetti' AND orders.processed IS true;